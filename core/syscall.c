...
int sys_getcwd(struct registers_t* regs) {
    ...
    if(vm_verify_write((u32)buf, size) < 0) {
        return -1;
    }
...
int sys_write(struct registers_t* regs) {
    ...
    if(vm_verify_read((u32)buf, cnt) < 0) {
        return -1;
    }
...
int sys_uname(struct registers_t* regs) {
    ...
    if(vm_verify_write((u32)p, sizeof(struct utsname)) < 0) {
        return -1;
    }
...
int sys_stat(struct registers_t* regs) {
    ...
    if(vm_verify_write((u32)stat, sizeof(struct stat)) < 0) {
        return -1;
    }
...
int sys_time(struct registers_t* regs) {
    ...
    if(vm_verify_write((u32)p, sizeof(struct tm)) < 0) {
        return -1;
    }
    *p = kern_time;
    return 1;
...
int sys_read(struct registers_t* regs) {
    ...
    u32 cnt   = regs->edx;
    s32 ret;
    if(vm_verify_write((u32)buf, cnt) < 0) {
        return -1;
    }
